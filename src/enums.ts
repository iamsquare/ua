/**
 * Named constants for common parser output strings.
 *
 * Prefer these with `@iamsquare/ua/helpers` (`isBrowser`, `isOS`, …) instead of raw string literals.
 *
 * @module @iamsquare/ua/enums
 */
import type { ValueOf } from 'type-fest';

import { BROWSER_TYPE, DEVICE_TYPE } from '@/types';

/** Browser category map (alias of core `BROWSER_TYPE`). */
export const BrowserType = BROWSER_TYPE;

/** Device form-factor map (alias of core `DEVICE_TYPE`). */
export const DeviceType = DEVICE_TYPE;

/** Known `browser.name` values. */
...
export type BrowserNameValue = ValueOf<typeof BrowserName>;

/** Known `cpu.architecture` values. */
...
export type CPUArchName = ValueOf<typeof CPUArch>;

/** Known `device.vendor` values. */
...
export type DeviceVendorValue = ValueOf<typeof DeviceVendor>;

/** Known `engine.name` values. */
...
export type EngineNameValue = ValueOf<typeof EngineName>;

/** Known `os.name` values. */
export const OSName = {
  AIX: 'AIX',
  AMIGA_OS: 'Amiga OS',
  ANDROID: 'Android',
...
export type OSNameValue = ValueOf<typeof OSName>;

/** Name maps used by extension packs (bots, apps, libraries, …). */
