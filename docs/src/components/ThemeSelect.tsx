...
const loadTheme = (): Theme =>
  parseTheme(typeof localStorage !== 'undefined' && localStorage.getItem(STORAGE_KEY));
...
export const ThemeSelect = ({ label }: ThemeSelectProps) => {
  ...
  return (
    <button
      type="button"
      className="m-0 flex cursor-pointer items-center justify-center border-0 bg-transparent p-8 text-(--sl-color-gray-2) hover:text-(--sl-color-white) [&_.label-icon]:size-16"
      aria-label={label}
      title={label}
