...
import { isAICrawler, isBot } from '@iamsquare/ua/bots';
import {
  bots,
  cli,
  crawler,
  ...
  mediaPlayer,
  vehicle,
} from '@iamsquare/ua/extensions';
import { isChromeFamily } from '@iamsquare/ua/helpers';
...
const ResultGridCell = ({ label, values }: ResultCell) => (
  <div className="grid h-full min-h-112 content-start justify-items-center gap-12 p-16">
    <span className={cn(labelPillClass, 'justify-self-start')}>{label}</span>
    <div className="flex flex-wrap items-center justify-center gap-8">
      {map(values, (value, index) => (
        <ResultPill key={`${label}-${index}`} value={value} />
      ))}
    </div>
  </div>
);

export const Playground = ({ showResults = true, showJson = false }: PlaygroundProps) => {
  ...
  const resultCells: ResultCell[] = [
    { label: 'Browser', values: pairValues(result.browser.name, result.browser.version) },
    { label: 'OS', values: pairValues(result.os.name, result.os.version) },
    { label: 'CPU Architecture', values: [result.cpu.architecture ?? '—'] },
    { label: 'Engine', values: pairValues(result.engine.name, result.engine.version) },
    { label: 'Device Type', values: [result.device.type ?? '—'] },
    {
      label: 'Device Vendor & Model',
      values: pairValues(result.device.vendor, result.device.model),
    },
    { label: 'Category', values: [result.browser.type ?? '—'] },
    { label: 'isChromeFamily()', values: [boolValue(isChromeFamily(result))] },
    { label: 'isBot()', values: [boolValue(isBot(result))] },
    { label: 'isAICrawler()', values: [boolValue(isAICrawler(result))] },
  ];
    ...
    <div className="not-content grid gap-20">
      ...
      <div className="grid gap-8 sm:grid-cols-[auto_1fr] sm:items-center">
        ...
        <select
          ...
          onChange={(event) => {
            ...
            setUa(next === 'custom' ? navigator.userAgent : next);
          }}
        >
          <option value="custom">Current browser (navigator.userAgent)</option>
          {map(PRESETS, (preset) => (
      ...
      {showResults && (
        <div
          className={cn(
            panelClass,
            'grid grid-cols-1 gap-px overflow-hidden bg-gray-200 p-0 sm:grid-cols-2 lg:grid-cols-12 dark:bg-gray-800',
          )}
        >
          {map(resultCells, (cell, index) => (
            <div
              key={cell.label}
              className={cn(
                'bg-white dark:bg-gray-950',
                index < 6 ? 'lg:col-span-4' : 'lg:col-span-3',
              )}
            >
              <ResultGridCell {...cell} />
            </div>
